...
IvPFunction *BHV_OpRegionRecover::onRunState() 
{
  ...
  double curr_hdg_dist_to_poly = m_polygon.dist_to_poly(m_osx, m_osy, m_osh);
...
bool BHV_OpRegionRecover::polygonVerify()
{
  ...
  bool contained = m_polygon.contains(m_osx, m_osy);
...
//   Purpose: To calculate information about impending OpRegionRecover
...
void BHV_OpRegionRecover::postPolyStatus()
{
  ...
  // current heading (CH).
  double trajectory_perim_dist = m_polygon.dist_to_poly(m_osx, m_osy, m_osh);
  ...
  double absolute_perim_dist = m_polygon.dist_to_poly(m_osx, m_osy);
...
void BHV_OpRegionRecover::updateRangeCache()
{
  ...
  for(unsigned int i=0; i<hdg_pts; i++) {
    ...
    double dist_to_region = m_polygon.dist_to_poly(m_osx, m_osy, heading);
