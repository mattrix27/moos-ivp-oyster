...
SHORE_IP=localhost #192.168.1.155
SHORE_LISTEN="9300"
...
############################
# behavior parameters (1/2)
START_A="158"   # degrees east of north
INITIAL_TURN_DIRECTION="right"
STRAIGHTAWAY_DURATION="30"
# RETURN_A set below
############################
...
for ARGI; do
    ...
    elif [ "${ARGI//[^0-9]/}" = "$ARGI" -a "$DURATION" = 1 ]; then
        STRAIGHTAWAY_DURATION=$ARGI
    else
...
############################
# behavior parameters (2/2)
BIAS_AMOUNT="45"
RETURN_A=$(( ($START_A+180) % 360 ))
if [ "${INITIAL_TURN_DIRECTION}" = "right" ] ; then
    LEFTHAND="false"
    # BIAS="+"$BIAS_AMOUNT
else
    LEFTHAND="true"
...
LH_FT=$(( ($START_A-$BIAS_AMOUNT) % 360 ))
RH_FT=$(( ($START_A+$BIAS_AMOUNT) % 360 ))
LH_NT=$(( ($RETURN_A-$BIAS_AMOUNT) % 360 ))
RH_NT=$(( ($RETURN_A+$BIAS_AMOUNT) % 360 ))
# BIAS_OUT_A=$(( ($START_A$BIAS) % 360 ))
# BIAS_RET_A=$(( ($RETURN_A$BIAS) % 360 ))

############################

echo "Going out at "$START_A" deg east of north"
echo "Returning at "$RETURN_A" deg east of north"
echo "Turning "${INITIAL_TURN_DIRECTION}
...
nsplug meta_m200.bhv targ_${VNAME}.bhv -f  \
        ...
        LH_NT_A=$LH_NT              \
        RH_NT_A=$RH_NT              \
        DUR=$STRAIGHTAWAY_DURATION  \
        INITIALLY_LEFTHAND=$LEFTHAND
...
pAntler targ_${VNAME}.moos --MOOSTimeWarp=10 >& /dev/null &
